...
class NeuralNetwork():
	...
	def Train(self, args):
		...
		# for r in open(args.filename[0]).read().splitlines():			
# 			train_list.append(r)
	...
	def PlotWeight(self):
		a = int(np.sqrt(self.NumOfHidden))	
		b = int(np.sqrt(self.NumOfInput))
		plotimg = np.zeros([a*b, a*b])
		for x in range(plotimg.shape[0]):
			for y in range(plotimg.shape[1]):
				plotimg[x][y] = self.w1[a*(x/b) + y/b][b*(x%b) + y%b]
